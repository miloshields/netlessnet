...
app.post('/start', async (req, res) => {
  ...
  // order of determining user mode
  //  (1) check if req object is
  if( await checkIfUserExists(mongoClient, req.body.From)) {
    req.session.userMode = "intro"
    if(req.session.introComplete){
      twiml.say("Ask away.")
    }
    else{
      twiml.say('Welcome to the Netless Net. Ask about the weather in a particular place, the solution to a math problem, or any general information or advice. You can also ask about news or for Wikipedia information on a topic. You can read to');
    }
    twiml.say("You can switch to expert mode")
  }
  // user should be in expert mode if they have
  else {
    req.session.userMode = "expert"
    twiml.say("Welcome to the Netless Net, Expert Mode. Press 0 to return to intro mode.");
  }
  twiml.gather({
    input: 'speech dtmf',
    action: '/get-resource',
